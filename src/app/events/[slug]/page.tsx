...
export default async function EventPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <Container className="max-w-3xl py-16">
      <span className="text-sm font-semibold uppercase text-crimson">{event.eventType}</span>
      <h1 className="mt-2 font-display text-3xl font-bold text-navy sm:text-4xl">{event.title}</h1>
      ...
      {event.image && (
        <div className="relative mt-8 aspect-video w-full overflow-hidden rounded-card">
          <Image src={urlFor(event.image).width(1200).url()} alt={event.title} fill className="object-cover" />
        </div>
      )}
      {event.summary && <p className="mt-8 text-ink-muted">{event.summary}</p>}
