...
export default async function PostPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <Container className="max-w-3xl py-16">
      <span className="text-sm font-semibold uppercase text-crimson">{post.category}</span>
      ...
      {post.mainImage && (
        <div className="relative mt-8 aspect-video w-full overflow-hidden rounded-card">
          <Image src={urlFor(post.mainImage).width(1200).url()} alt={post.title} fill className="object-cover" />
        </div>
      )}
